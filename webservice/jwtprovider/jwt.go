...
const (
	...
	configRootJwtSecurityProvider = "security.keys.jwt"
)

type JwtSecurityProviderConfig struct {
	KeyPath string   `config:"default=secret/phi_pnp"`
	KeyName string   `config:"default=key"`
	Rules   []string `config:"default=/api/**;/admin;/admin/**"`
	Exceptions []string `config:"default=/admin/health;/admin/info"`
}
...
func (f *JwtSecurityProvider) requiresAuthentication(path string) (bool, error) {
	blacklisted := false
	...
	for _, rule := range f.cfg.Exceptions {
		if matches, err := doublestar.Match(rule, path); err != nil {
			return true, err
		} else if matches {
			return false, nil
...
func (f *JwtSecurityProvider) Authentication(req *restful.Request) (err error) {
	...
	var required bool
	if required, err = f.requiresAuthentication(path); err != nil {
		return err
	}
	...
	// 3. Inject the security context if we validated the token
	if !invalid {
